...
def routing(votes,
    ...
    if vote_rank != 5 and vote_rank != 7:
        raise ValueError(
            'Votes to "routing" should have rank 5 or 7 but it is rank', str(vote_rank))
    if activation_rank != 2 and activation_rank != 4:
        raise ValueError(
            'Activation to "routing" should have rank 2 or 4 but it is rank', str(activation_rank))
    ...
    def _body(i, logits, poses):
        ...
        if use_bias:
            preactivate = cl.reduce_sum(
                route * votes, axis=-4, keepdims=True) + biases
        else:
        ...
        if vote_shape[-1] == 1:
            ...
            diff = votes - pose
            distances = tf.linalg.trace(cl.matmul(diff, diff)
...
def dynamicRouting_v1(votes,
    ...
    if use_bias:
        bias_shape = [1 for i in range(len(vote_shape) - 3)] + vote_shape[-3:]
        biases = tf.Variable(initial_value=0.1,
                             name="biases",
                             shape=bias_shape,
                             dtype=tf.float32)
    ...
    for i in range(num_routing):
        with tf.name_scope("iter_" + str(i)):
            if leaky:
                route = _leaky_routing(logits)
            else:
            ...
            if i == num_routing - 1:
                if use_bias:
                    preactivate = cl.reduce_sum(tf.multiply(
                        route, votes), axis=-4, keepdims=True) + biases
                else:
                    preactivate = cl.reduce_sum(tf.multiply(
                        route, votes), axis=-4, keepdims=True)
                poses = cl.ops.squash(preactivate, axis=squash_on)
            else:
                if use_bias:
                    preactivate = cl.reduce_sum(tf.multiply(
                        route, vote_stopped), axis=1, keepdims=True) + biases
                else:
                    preactivate = cl.reduce_sum(tf.multiply(
                        route, vote_stopped), axis=1, keepdims=True)
                poses = cl.ops.squash(preactivate, axis=squash_on)
                logits += cl.reduce_sum(vote_stopped *
                                        poses, axis=-4, keepdims=True)
...
def EMRouting(votes,
    ...
    for t_iter in range(num_routing):
        ...
        inverse_temperature = lambda_min + \
            (lambda_max - lambda_min) * t_iter / max(1.0, num_routing)
        with tf.name_scope('M-STEP') as scope:
            # if t_iter > 0:
            #     scope.reuse_variables()
            pose, log_var, log_activation_prime = M_step(
                log_R, log_activation, votes, lambda_val=inverse_temperature)
    ...
    pose = tf.reshape(
        pose, shape=vote_shape[:-4] + [num_outputs] + out_caps_dims)
...
def M_step(log_R, log_activation, vote, lambda_val=0.01):
    ...
    log_var = tf.reduce_logsumexp(
        log_normalized_R + cl.log(tf.square(vote - pose)), axis=-3, keepdims=True)
    shape = [1 for i in range(len(pose.shape) - 2)] + [pose.shape[-2], 1]
    beta_v = tf.Variable(initial_value=tf.keras.initializers.TruncatedNormal(mean=15., stddev=3.)(shape),
                         name="beta_v",
                         shape=shape)
    cost = R_sum_i * (beta_v + 0.5 * log_var)

    beta_a = tf.Variable(initial_value=tf.keras.initializers.TruncatedNormal(mean=100.0, stddev=10)(shape),
                         name="beta_a",
                         shape=shape)
...
def E_step(pose, log_var, log_activation, vote):
    ...
    log_R = log_activation_logit - \
        tf.reduce_logsumexp(log_activation_logit, axis=-2, keepdims=True)
    return log_R


class Routing(tf.keras.layers.Layer):
    def __init__(self, method='EMRouting', num_iter=3, use_bias=True,
                 leaky=True, name='routing', **kwargs):
    ...
    def build(self, vote_shape):
        if self.method == 'DynamicRouting' and self.use_bias:
            bias_shape = [1 for i in range(
                len(vote_shape) - 3)] + vote_shape[-3:]
            self.biases = tf.Variable(initial_value=tf.constant_initializer(0.1)(bias_shape, tf.float32),
                                      name="biases")

        if self.method == 'EMRouting':
            pose_shape = vote_shape[:-4] + vote_shape[-3:]
            shape = [1 for i in range(len(pose_shape) - 2)] + \
                [pose_shape[-3], 1]
            self.beta_v = tf.Variable(initial_value=tf.keras.initializers.TruncatedNormal(mean=15., stddev=3.)(shape),
                                 name="beta_v",
                                 shape=shape)
            self.beta_a = tf.Variable(initial_value=tf.keras.initializers.TruncatedNormal(mean=100.0, stddev=10)(shape),
                                 name="beta_a",
                                 shape=shape)
    ...
    def dynamicRouting(self, votes):
        ...
        poses = tf.TensorArray(dtype=tf.float32,
                               ...
                               clear_after_read=False)
        i = tf.constant(0, dtype=tf.int32)
        _, logits, poses = tf.while_loop(lambda i, logits, poses: i < self.num_iter,
                                         _body,
                                         loop_vars=[i, logits, poses],
                                         swap_memory=True)
    ...
    def M_step(self, log_R, log_activation, vote, lambda_val=0.01):
        ...
        log_var = tf.reduce_logsumexp(log_normalized_R +
                                      cl.log(tf.square(vote - pose)), axis=-3, keepdims=True)
    ...
    def call(self, votes, activation=None):
        ...
        if self.method == 'EMRouting':
            pose, activation = self.EMRouting(votes, activation)
